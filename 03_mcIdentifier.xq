...
(:declare variable $motscomp := file:read-text-lines('/Users/xavier-laurentsalvador/basex/listeOccurrencesMotsComposesTerre.txt');:)
(:ce fichier est produit par le script csvMotsCOmposés.xq:)
(:35619 ms - 40 secondes mais 4 minutes de production de fichier:)

declare variable $motscomp := file:read-text-lines('/Users/xavier-laurentsalvador/basex/listeOccurrenceMCcandidats.txt');
...
for $p in $corpus
 return
  if (string-join($p//text(),' ') contains text {$motif} all window ($nb + 1) words)
   then
  <p src="{$m}" pos="{if ($bddMotsComp[./@src=$m]/@pos) then distinct-values($bddMotsComp[./@src=$m]/@pos) else 'XXX'}" index="{$p/@index}">
   {
